...
class bankaraopen(commands.Cog):
    ...
    async def loop(self):
            ...
            embed = discord.Embed(title='バンカラオープン',color=0xdb4a13,
    ...
    async def on_message(self,message):
        ...
        if message.content.lower() in ['バンカラオープン', 'ばんからおーぷん', 'open', 'bankaraopen', 'オープン', 'おーぷん']:
            url = "https://spla3.yuu26.com/api/bankara-open/schedule"
            ua = "Splatoon3/ikacord bot (twitter @Mt_PheyK, Discord PheyK#1280"
            headers = {"User-Agent": ua}
            response = requests.get(url)
        ...
        if message.content.lower() in ['次のバンカラオープン', 'つぎのばんからおーぷん', 'nextopen', 'nextbankaraopen', '次のオープン', 'つぎのおーぷん']:
        ...
        if message.content.lower() in ['次の次のバンカラオープン', 'つぎのつぎのばんからおーぷん', 'afternextopen', 'afternextbankaraopen', '次の次のオープン', 'つぎのつぎのおーぷん']:
            url1 = "https://spla3.yuu26.com/api/bankara-open/schedule"
            response = requests.get(url1)
            jsonData = response.json()
    ...
    @slash_command(name='stageオープン', guild_ids = guild_ids, description='バンカラオープンのステージ情報を取得します。')
