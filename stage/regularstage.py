...
class regularstage(commands.Cog):
    ...
    async def loop(self):
            ...
            if now == '09:00' or now == '21:00':
                channel = self.bot.get_channel(1031555899145326653)
    ...
    async def on_message(self,message):
        ...
        if message.content.lower() in ['レギュラーマッチ', 'れぎゅらーまっち', 'regular', 'レギュラー', 'れぎゅらー', 'ナワバリ', 'なわばり']:
            url = "https://spla3.yuu26.com/api/regular/schedule"
            ua = "Splatoon3/ikacord bot (twitter @Mt_PheyK, Discord PheyK#1280"
            headers = {"User-Agent": ua}
            response = requests.get(url)
        ...
        if message.content.lower() in ['次のレギュラーマッチ', 'つぎのれぎゅらーまっち', 'nextregular', '次のレギュラー', 'つぎのれぎゅらー', '次のナワバリ', 'つぎのなわばり']:
        ...
        if message.content.lower() in ['次の次のレギュラーマッチ', 'つぎのつぎのれぎゅらーまっち', 'afternextregular', '次の次のレギュラー', 'つぎのつぎのれぎゅらー', '次の次のナワバリ', 'つぎのつぎのなわばり']:
            url2 = "https://spla3.yuu26.com/api/regular/schedule"
    ...
    @slash_command(name='stageレギュラー', guild_ids = guild_ids, description='レギュラーのステージ情報を取得します。')
