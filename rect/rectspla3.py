...
class rectspla(discord.ui.Modal):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        img = Image.open("images/rect/nanndemo.png")
        ...
        await interaction.followup.send(f"{interaction.user.mention}が募集中！: {spla3.is_persistent(spla3())}", embed=embed, file=f, view=spla3())

class splacom(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @slash_command(name="rectなんでも" ,guild_ids=guild_ids, description="スプラ関連なんでも募集を取り付けます。")
    async def splarect(self, interaction: discord.Interaction):
        if interaction.channel.id not in [1027901079368761426,1027901127099953152,1027901157076631582,1027901192724029440,1029001205076209755,802345513495822339,803028814736392192]:
            return await interaction.respond("エラー：なんでも募集コマンドは <#1027901079368761426> <#1027901127099953152> <#1027901157076631582> <#1027901192724029440> <#1029001205076209755>で実行して下さい。",ephemeral = True)  
        modal = rectspla(title="募集の詳細を説明")
        await interaction.response.send_modal(modal)

    @commands.Cog.listener()
    async def on_message(self,message):
        if message.author.bot:
            if message.channel.id not in [1027901079368761426,1027901127099953152,1027901157076631582,1027901192724029440,1029001205076209755,802345513495822339,803028814736392192]:
                return
            rectword=['が募集中！']
