...
class feststage(commands.Cog):
    ...
    async def loop(self):
            ...
            embed = discord.Embed(title='フェスマッチ',color=0x00ff00,
                                 description=f"**{n}～{n2}**\n**{map}/{map2}**\n\n"
                                 f"**{n3}～{n4}**\n**{map3}/{map4}**\n\n"
                                 f"**{n5}～{n6}**\n**{map5}/{map6}**\n\n"
                                 f"**{n7}～{n8}**\n**{map7}/{map8}**\n\n"
            ...
            if now == '09:00' or now == '21:00':
                channel = self.bot.get_channel(1040641430143651900)
    ...
    async def on_message(self,message):
        ...
        if message.content.lower() in ['フェスマッチ', 'ふぇすまっち', 'fest', 'フェス', 'ふぇす', 'トリカラ', 'とりから']:
            url = "https://spla3.yuu26.com/api/fest/schedule"
            ua = "Splatoon3/ikacord bot (twitter @Mt_PheyK, Discord PheyK#1280"
            headers = {"User-Agent": ua}
            response = requests.get(url)
        ...
        if message.content.lower() in ['次のフェスマッチ', 'つぎのふぇすまっち', 'nextfest', '次のフェス', 'つぎのふぇす', '次のトリカラ', 'つぎのとりから']:
            url1 = "	https://spla3.yuu26.com/api/fest/schedule"
            ...
            embed2.set_thumbnail(url="https://img.gamewith.jp/img/17cd16891d1d50e569e19879057dfa26.png")
            embed2.set_footer(text="API: https://spla3.yuu26.com| イカコード3")
            ctx = await self.bot.get_context(message)

            await ctx.send(embed=embed2,file=file1)

        if message.content.lower() in ['次の次のフェスマッチ', 'つぎのつぎのふぇすまっち', 'nextafterfest', '次の次のフェス', 'つぎのつぎのふぇす', '次の次のトリカラ', 'つぎのつぎのとりから']:
            url2 = "	https://spla3.yuu26.com/api/fest/schedule"
            ...
            embed3.set_thumbnail(url="https://img.gamewith.jp/img/17cd16891d1d50e569e19879057dfa26.png")
