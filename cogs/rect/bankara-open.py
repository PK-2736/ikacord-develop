...
class rectbankara(discord.ui.Modal):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        img = Image.open("images/rect/bannkara.png")
        ...
        await interaction.followup.send(f"{interaction.user.mention}がバンカラ募集中！: {spla3.is_persistent(spla3())}", embed=embed, file=f, view=spla3())

class bankaracom(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @slash_command(name="rectバンカラ" ,guild_ids=guild_ids, description="バンカラ募集を取り付けます。")
    async def bankararect(self, interaction: discord.Interaction):  
        if interaction.channel.id not in [1027901127099953152,802345513495822339,803028814736392192]:
    ...
    async def on_message(self,message):
        if message.author.bot:
            if message.channel.id not in [1027901127099953152,802345513495822339,803028814736392192]:
                return
            rectword=['がバンカラ募集中！']
