...
class rectcoop(discord.ui.Modal):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        img = Image.open("images/rect/salmon.png")
        ...
        await interaction.followup.send(f"{interaction.user.mention}がバイト募集中！: {spla3.is_persistent(spla3())}", embed=embed, file=f, view=spla3())

class coopcom(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @slash_command(name="ぼしゅうバイト" ,guild_ids=guild_ids, description="バイト募集を取り付けます。")
    async def cooprect(self, interaction: discord.Interaction):
        if interaction.channel.id not in [1027901192724029440,802345513495822339,803028814736392192]:
            return await interaction.respond("エラー：バンカラ募集コマンドは <#1027901192724029440> で実行して下さい。",ephemeral = True)  
    ...
    async def on_message(self,message):
        if message.author.bot:
            if message.channel.id not in [1027901192724029440,802345513495822339,803028814736392192]:
                return
            rectword=['がバイト募集中！']
