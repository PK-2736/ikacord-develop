...
class recthuuka(discord.ui.Modal):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        img = Image.open("images/rect/huuka.png")
        ...
        for line in wrap_list:
            ...
            draw.multiline_text((260, y),line, fill=(77,255,111), font=font2)
        ...
        await interaction.followup.send(f"{interaction.user.mention}が<@&1037356236393422899>募集中！: {spla3.is_persistent(spla3())}", embed=embed, file=f, view=spla3())

class huukacmd(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @slash_command(name="festくさ陣営" ,guild_ids=guild_ids, description="くさ陣営のフェス募集を取り付けます。")
    async def huukarect(self, interaction: discord.Interaction):  
        if interaction.channel.id not in [1038037274824278116,802345513495822339,803028814736392192]:
            return await interaction.respond("エラー：くさ陣営募集コマンドは <#1038037274824278116> で実行して下さい。",ephemeral = True)   
    ...
    async def on_message(self,message):
        if message.author.bot:
            if message.channel.id not in [1038037274824278116,802345513495822339,803028814736392192]:
                return
            rectword=['が<@&1037356236393422899>募集中！']
            for word in rectword:
                if word in message.content:
                    await message.channel.send("<@&983297498271580170><@&1037356236393422899>", delete_after=5)
            return 
