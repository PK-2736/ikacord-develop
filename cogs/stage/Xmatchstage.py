...
class Xmatch(commands.Cog):
    ...
    async def loop(self):
            ...
            embed = discord.Embed(title='Xマッチ',color=0x14e494,
            ...
            embed.set_thumbnail(url="https://github.com/PK-2736/ikacord-develop/blob/823235430a2f1008f56d998988dd10fb1d190d0e/images/logo/xmatch.png?raw=true")
    ...
    async def on_message(self,message):
        ...
        if message.content.lower() in ['Xマッチ', 'Xまっち', 'Xmatch', 'xmatch', 'xマッチ', 'xまっち']:
            url = "https://spla3.yuu26.com/api/x/schedule"
            ua = "Splatoon3/ikacord bot (twitter @Mt_PheyK, Discord PheyK#1280"
            headers = {"User-Agent": ua}
            response = requests.get(url)
        ...
        if message.content.lower() in ['次のXマッチ', 'つぎのXまっち', 'nextXmatch', 'つぎのXマッチ', 'nextxmatch', 'つぎのxまっち']:
            url1 = "https://spla3.yuu26.com/api/x/schedule"
            ...
            embed2.set_thumbnail(url="https://github.com/PK-2736/ikacord-develop/blob/823235430a2f1008f56d998988dd10fb1d190d0e/images/logo/xmatch.png?raw=true")
            embed2.set_footer(text="API: https://spla3.yuu26.com| イカコード3")
            ctx = await self.bot.get_context(message)

            await ctx.send(embed=embed2,file=file1)

        if message.content.lower() in ['次の次のXマッチ', 'つぎのつぎのXまっち', 'afternextXmatch', 'afternextxマッチ', '次の次のxマッチ', 'つぎのつぎのxまっち']:
            url1 = "https://spla3.yuu26.com/api/x/schedule"
            response = requests.get(url1)
            jsonData = response.json()
            ...
            embed3.set_thumbnail(url="https://github.com/PK-2736/ikacord-develop/blob/823235430a2f1008f56d998988dd10fb1d190d0e/images/logo/xmatch.png?raw=true")
            embed3.set_footer(text="API: https://spla3.yuu26.com| イカコード3")
            ctx = await self.bot.get_context(message)

            await ctx.send(embed=embed3,file=file2)
    
    @slash_command(name='すてxマッチ', guild_ids = guild_ids, description='Xマッチのステージ情報を取得します。')
    async def Xmatch(self, ctx, time : Option(str, '時間', choices=["今", "次", "次の次"])):
        await ctx.defer()
        ...
        if time == "今":
            ...
            file = discord.File(img_binary, filename='image.png')
            embed.set_image(url="attachment://image.png")
            embed.set_thumbnail(url="https://github.com/PK-2736/ikacord-develop/blob/823235430a2f1008f56d998988dd10fb1d190d0e/https://github.com/PK-2736/ikacord-develop/blob/823235430a2f1008f56d998988dd10fb1d190d0e/images/logo/xmatch.png?raw=true?raw=true")
            embed.set_footer(text="API: https://spla3.yuu26.com| イカコード3")
